...
When MESHWORK_CALLBACK_URL is set (for interactive sessions outside the runner),
...
def main():
    try:
        ...
        callback_url = os.environ.get("MESHWORK_CALLBACK_URL")
        ...
        payload = json.dumps({
            "source": "meshwork-plugin-stop-hook",
            "sessionId": session_id,
            "stopReason": stop_reason,
        }).encode("utf-8")

        secret = os.environ.get("MESHWORK_CALLBACK_SECRET", "")
        headers = {
        ...
        print(json.dumps({"systemMessage": f"Meshwork on_stop warning: {e}"}))
